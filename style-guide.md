...
- The map is for orientation only. Each day provides a downloadable KML bookmark collection generated from that day's numbered POI legend; importing it once keeps those guide POIs visible as persistent bookmarks in Organic Maps' simplified driving view. Organic Maps' stable two-point route links provide POI-visible navigation and ordered multi-stop routes must be split into explicit legs. Google Maps complete-route buttons must remain available as fallbacks because the stable Organic Maps app cannot receive an ordered multi-stop route in one link.
...
- Every page must validate that every POI appears on the map, every POI appears in the legend and downloadable KML in the same order, numbering is unique, marker and legend numbering match, addresses are complete, coordinates are verified, Organic Maps POI and two-point route links use those verified coordinates where available, no unsupported Organic Maps v2 route links exist, and Google Maps complete-route fallbacks continue to function.
